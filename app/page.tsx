...
type MarketAgent = {
  ...
  tags: string[];
  pilot?: boolean;
  teamPreset?: "marketing" | "ceo";
  subagents?: Array<{ name: string; role: string; provider: string; model: string; capability: "text" | "research" | "image" | "reasoning" }>;
};
...
type AgentModelRoute = { provider: "openai" | "google"; providerLabel: "OpenAI" | "Google Gemini"; model: string; capability: "text" | "research" | "image" | "reasoning" };
type TeamAgent = { id: string; name: string; role: string; purpose: string; icon: string; accent: string; tools: string[]; inputs: string[]; outputs: string[]; guardrails: string[]; model?: AgentModelRoute };
...
const catalog: MarketAgent[] = [
  { id: "milana-marketing", name: "Marketing Agent", tagline: "One brief becomes a complete, measurable campaign", description: "A production-ready marketing lead with specialist subagents for research, strategy, copy, image generation, distribution, and performance learning.", category: "Marketing", icon: "M", accent: "pink", creator: "Milana Agent Studio", installs: 0, rating: 5, verified: true, pilot: true, teamPreset: "marketing", tags: ["Campaigns", "Copy", "Images", "Analytics"], subagents: [
    { name: "Growth Director", role: "Orchestration and approvals", provider: "OpenAI", model: "gpt-5", capability: "reasoning" },
    { name: "Audience Researcher", role: "Audience and competitor intelligence", provider: "OpenAI", model: "gpt-5 + web search", capability: "research" },
    { name: "Campaign Copywriter", role: "Channel-ready text and variants", provider: "OpenAI", model: "gpt-5", capability: "text" },
    { name: "Visual Creator", role: "Campaign imagery and creative directions", provider: "Google Gemini", model: "gemini-2.5-flash-image", capability: "image" },
    { name: "Performance Analyst", role: "Measurement and learning loop", provider: "OpenAI", model: "gpt-5", capability: "reasoning" },
  ] },
  { id: "milana-ceo", name: "CEO Assistant", tagline: "An executive office that researches, briefs, and follows through", description: "A chief-of-staff agent with specialist subagents for executive research, decision support, meeting preparation, writing, and follow-up.", category: "Executive", icon: "C", accent: "indigo", creator: "Milana Agent Studio", installs: 0, rating: 5, verified: true, pilot: true, teamPreset: "ceo", tags: ["Briefings", "Decisions", "Meetings", "Follow-up"], subagents: [
    { name: "Chief of Staff", role: "Prioritization, routing, and synthesis", provider: "OpenAI", model: "gpt-5", capability: "reasoning" },
    { name: "Executive Researcher", role: "Evidence-backed company and market research", provider: "OpenAI", model: "gpt-5 + web search", capability: "research" },
    { name: "Decision Analyst", role: "Options, trade-offs, and risk analysis", provider: "OpenAI", model: "gpt-5", capability: "reasoning" },
    { name: "Briefing Writer", role: "Memos, updates, and executive communications", provider: "OpenAI", model: "gpt-5", capability: "text" },
    { name: "Meeting & Follow-up Agent", role: "Agendas, preparation, and action tracking", provider: "OpenAI", model: "gpt-5", capability: "text" },
  ] },
...
const categories = ["All", "Executive", "Marketing", "Customer support", "Sales", "Research", "Data", "Engineering", "People", "Commerce"];
const quickPrompts = [
...
export default function AgentMarketplace() {
  ...
  const useTemplate = async () => {
    ...
    if (selectedAgent.teamPreset) {
      setBusy(true);
      try {
        const response = await fetch("/api/teams", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "install-preset", preset: selectedAgent.teamPreset }) });
        const payload = await response.json() as { team?: AgentTeam; error?: string };
        if (!response.ok || !payload.team) throw new Error(payload.error || "The pilot team could not be installed.");
        setSavedTeams((current) => [payload.team!, ...current.filter((item) => item.id !== payload.team!.id)]);
        setActiveTeam(payload.team);
        setView("teams");
        notify(`${payload.team.name} installed with ${payload.team.agents.length} live roles`);
      } catch (error) { notify(error instanceof Error ? error.message : "Pilot installation failed", "neutral"); }
      finally { setBusy(false); }
      return;
    }
    setMessages(initialMessages);
...
function Sidebar({ view, setView, open, setOpen, onNew, savedAgents, savedTeams }: { view: View; setView: (view: View) => void; open: boolean; setOpen: (open: boolean) => void; onNew: () => void; savedAgents: Blueprint[]; savedTeams: AgentTeam[] }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "open" : ""}`}>
      <div className="brand-row"><button className="brand" onClick={() => navigate("marketplace")}><span className="brand-symbol">✦</span><strong>Milana Agent Studio</strong></button><button className="mobile-close" onClick={() => setOpen(false)} aria-label="Close menu">×</button></div>
...
function Marketplace({ query, setQuery, category, setCategory, filtered, onCreate, onTeam, onAudition, prompt, setPrompt, onInstall }: { query: string; setQuery: (value: string) => void; category: string; setCategory: (value: string) => void; filtered: MarketAgent[]; onCreate: (value: string) => void; onTeam: () => void; onAudition: () => void; prompt: string; setPrompt: (value: string) => void; onInstall: (agent: MarketAgent) => void }) {
  ...
  const pilotAgents = catalog.filter((agent) => agent.pilot);
  return <div className="marketplace-view">
    <section className="market-hero">
      <div className="hero-glow glow-one" /><div className="hero-glow glow-two" />
      <div className="hero-copy"><span className="eyebrow"><i /> MILANA AGENT STUDIO</span><h1>Real agents for real work.<br/><span>Specialists, models, and handoffs included.</span></h1><p>Start with a pilot-ready agent team or describe your own. Every agent has a clear role, an assigned model, explicit guardrails, and reviewable handoffs.</p></div>
    ...
    <section className="market-content">
      {category === "All" && !query && <section className="pilot-section"><div className="pilot-heading"><div><span className="eyebrow"><i /> READY FOR THE PILOT</span><h2>Milana agent teams</h2><p>Install a complete agent with production-shaped subagents and model routing.</p></div><span className="pilot-status"><i /> 2 teams ready</span></div><div className="pilot-grid">{pilotAgents.map((agent) => <button className="pilot-card" key={agent.id} onClick={() => onInstall(agent)}><div className="pilot-card-head"><span className={`agent-icon xl ${agent.accent}`}>{agent.icon}</span><div><span>{agent.category.toUpperCase()} AGENT</span><strong>{agent.subagents?.length} live roles</strong></div><i>→</i></div><h3>{agent.name}</h3><p>{agent.description}</p><div className="pilot-models">{Array.from(new Set(agent.subagents?.map((subagent) => subagent.provider))).map((provider) => <span key={provider}>{provider}</span>)}</div><div className="pilot-subagents">{agent.subagents?.slice(0, 4).map((subagent) => <span key={subagent.name}><i>{subagent.name[0]}</i>{subagent.name}</span>)}<span><i>+</i>Full team</span></div></button>)}</div></section>}
      ...
      <div className="agents-grid">{filtered.filter((agent) => !(category === "All" && !query && (agent.featured || agent.pilot))).map((agent) => <AgentCard key={agent.id} agent={agent} onClick={() => onInstall(agent)} />)}</div>
  ...
  return <div className={`builder-view ${blueprint ? "has-blueprint" : ""}`}>
    <section className="chat-pane">
      <div className="conversation"><div className="conversation-inner">{messages.map((message) => <div key={message.id} className={`message ${message.role}`}>
        ...
        <div className="message-body">{message.role === "architect" && <strong>Milana Architect</strong>}<p>{message.text}</p>{message.id === "welcome" && messages.length === 1 && <div className="builder-suggestions">{quickPrompts.map((item) => <button key={item} onClick={() => createAgent(item)}><span>✦</span>{item}<i>→</i></button>)}</div>}</div>
      </div>)}{busy && <div className="message architect"><span className="architect-avatar thinking">✦</span><div className="message-body"><strong>Milana Architect</strong><div className="thinking-line"><i/><i/><i/><span>Designing the blueprint</span></div></div></div>}</div></div>
      <div className="chat-composer-wrap"><form className="chat-composer" onSubmit={submit}><textarea ref={composerRef} value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); if (prompt.trim()) createAgent(prompt); } }} placeholder={blueprint ? `Ask to refine ${blueprint.name}…` : "Describe your agent…"} rows={2} aria-label="Message Milana Architect"/><div className="chat-composer-bottom"><div><button type="button" className="round-tool" aria-label="Attach file">＋</button><button type="button" className={`web-toggle ${webEnabled ? "on" : ""}`} onClick={() => setWebEnabled(!webEnabled)}><span>◎</span> Web</button><label className="model-picker"><select value={model} onChange={(event) => setModel(event.target.value)} aria-label="Builder model"><option>Auto</option><option>Fast</option><option>Powerful</option></select></label></div><div><button type="button" className="mic-button" aria-label="Voice input">◉</button><button className="send-button" aria-label="Send message" disabled={!prompt.trim() || busy}>↑</button></div></div></form><p className="composer-note">Milana Architect can make mistakes. Review tools and permissions before publishing.</p></div>
    </section>
...
function TeamStudio({ teams, setTeams, activeTeam, setActiveTeam, model, notify }: { teams: AgentTeam[]; setTeams: React.Dispatch<React.SetStateAction<AgentTeam[]>>; activeTeam: AgentTeam | null; setActiveTeam: (team: AgentTeam | null) => void; model: string; notify: (text: string, tone?: "success" | "neutral") => void }) {
  ...
  const [runPrompt, setRunPrompt] = useState("Create a concise first-pass deliverable for this role using the team objective and clearly mark any assumptions.");
  const [runResult, setRunResult] = useState<{ kind: "text" | "image"; text?: string; data?: string; mimeType?: string; provider?: string; model?: string; error?: string } | null>(null);
  ...
  const runAgent = async () => {
    if (!activeTeam || !runAgentId || !runPrompt.trim()) return;
    setRunning(true);
    setRunResult(null);
    try {
      const response = await fetch("/api/agents/run", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ teamId: activeTeam.id, agentId: runAgentId, input: runPrompt }) });
      const payload = await response.json() as { kind?: "text" | "image"; text?: string; data?: string; mimeType?: string; provider?: string; model?: string; error?: string };
      if (!response.ok || !payload.kind) throw new Error(payload.error || "The agent run failed.");
      setRunResult(payload as typeof runResult);
      notify("Live agent run completed");
    } catch (error) { setRunResult({ kind: "text", error: error instanceof Error ? error.message : "The agent run failed." }); }
  ...
  if (!activeTeam) return <div className="team-create-view">
    <section className="team-create-hero"><div><span className="eyebrow"><i /> MULTI-AGENT CREATOR</span><h1>Build the team,<br/><span>not just the agents.</span></h1><p>Start with one outcome. Milana Architect researches the domain, defines the right specialists, assigns the right models, and wires their handoffs into one working system.</p></div><div className="team-architecture-preview"><div className="preview-core">✦<small>Lead</small></div>{["Research", "Strategy", "Create", "Measure"].map((item, index) => <div key={item} className={`preview-member pm-${index + 1}`}><i>{item[0]}</i><span>{item}</span></div>)}</div></section>
  ...
  return <div className="team-workbench"><header className="team-workbench-head"><div className="team-title-row"><button className="back-teams" onClick={() => setActiveTeam(null)}>←</button><div className="team-stack small">{activeTeam.agents.slice(0, 4).map((agent, index) => <i key={agent.id} className={agent.accent} style={{ zIndex: 5 - index }}>{agent.icon}</i>)}</div><div><span>AGENT TEAM</span><h1>{activeTeam.name}</h1></div><span className={`team-status ${activeTeam.status}`}><i /> {activeTeam.status}</span></div><div><button className="save-team" onClick={() => persist("save")} disabled={busy}>Save</button><button className="publish-team" onClick={() => persist("publish")} disabled={busy}>{activeTeam.status === "published" ? "Published ✓" : "Publish team"}</button></div></header>
    ...
    <div className="team-stage-body">
      ...
      {stage === "members" && <div className="members-stage">
        <div className="stage-heading-row"><div><span className="eyebrow"><i /> BOUNDED OWNERSHIP</span><h2>{activeTeam.agents.length} specialists, one shared outcome</h2><p>Each agent owns a distinct responsibility, model route, and explicit input/output contract.</p></div><button onClick={addMember}>＋ Add agent</button></div>
        <div className="team-member-grid">{activeTeam.agents.map((agent, index) => <article className="team-member-card" key={agent.id}>
          <div className="member-card-top"><span className={`agent-icon ${agent.accent}`}>{agent.icon}</span><div><span>{index === 0 ? "ORCHESTRATOR" : `SPECIALIST ${String(index).padStart(2, "0")}`}</span><input value={agent.name} onChange={(event) => updateMember(agent.id, "name", event.target.value)}/></div><button onClick={() => removeMember(agent.id)} disabled={activeTeam.agents.length <= 2}>×</button></div>
          <div className={`model-route ${agent.model?.provider || "openai"}`}><span>{agent.model?.providerLabel || "OpenAI"}</span><strong>{agent.model?.model || "gpt-5"}</strong><i>{agent.model?.capability || "text"}</i></div>
          <label><span>Role</span><input value={agent.role} onChange={(event) => updateMember(agent.id, "role", event.target.value)}/></label><label><span>Purpose</span><textarea value={agent.purpose} onChange={(event) => updateMember(agent.id, "purpose", event.target.value)} rows={3}/></label>
          <div className="member-contract"><div><span>INPUTS</span>{agent.inputs.slice(0, 3).map((item) => <i key={item}>{item}</i>)}</div><b>→</b><div><span>OUTPUTS</span>{agent.outputs.slice(0, 3).map((item) => <i key={item}>{item}</i>)}</div></div><div className="member-tools"><span>TOOLS</span>{agent.tools.map((tool) => <i key={tool}>{tool}</i>)}</div>
          <button className="test-agent-button" onClick={() => { setRunAgentId(agent.id); setRunResult(null); }}>Test this agent <span>→</span></button>
        </article>)}</div>
        {runAgentId && <section className="agent-runner"><div className="agent-runner-head"><div><span className="eyebrow"><i /> LIVE MODEL TEST</span><h3>Run {activeTeam.agents.find((agent) => agent.id === runAgentId)?.name}</h3><p>This sends the task only to the selected subagent using its assigned provider and role guardrails.</p></div><button onClick={() => { setRunAgentId(""); setRunResult(null); }} aria-label="Close agent test">×</button></div><textarea value={runPrompt} onChange={(event) => setRunPrompt(event.target.value)} rows={4} aria-label="Agent test task"/><div className="agent-runner-actions"><span>{activeTeam.agents.find((agent) => agent.id === runAgentId)?.model?.providerLabel || "OpenAI"} · {activeTeam.agents.find((agent) => agent.id === runAgentId)?.model?.model || "gpt-5"}</span><button onClick={runAgent} disabled={running || !runPrompt.trim()}>{running ? "Running agent…" : "Run live test"}</button></div>{runResult?.error && <div className="run-error"><strong>Connection required</strong><p>{runResult.error}</p></div>}{runResult?.kind === "text" && runResult.text && <div className="run-output"><div><strong>{runResult.provider} · {runResult.model}</strong><span>Live result</span></div><p>{runResult.text}</p></div>}{runResult?.kind === "image" && runResult.data && <div className="run-image-output"><img src={`data:${runResult.mimeType || "image/png"};base64,${runResult.data}`} alt="Generated campaign creative"/><div><strong>{runResult.provider} · {runResult.model}</strong><p>{runResult.text || "Image generated by the selected visual subagent."}</p></div></div>}</section>}
        <button className="next-stage" onClick={() => setStage("wiring")}>Wire the handoffs <span>→</span></button>
      </div>}
...
function Library({ agents, onOpen, onNew }: { agents: Blueprint[]; onOpen: (agent: Blueprint) => void; onNew: () => void }) {
  return <div className="standard-view"><div className="page-heading"><div><span className="eyebrow"><i /> YOUR WORKSPACE</span><h1>My agents</h1><p>Draft, evaluate, version, and manage every Agent Package you create.</p></div><button className="primary-page-action" onClick={onNew}>＋ Create agent</button></div>{agents.length ? <div className="library-grid">{agents.map((agent) => <button className="library-card" key={agent.id} onClick={() => onOpen(agent)}><div className="library-card-head"><div className={`agent-icon ${agent.accent}`}>{agent.icon}</div><span className={`library-status ${agent.status}`}><i /> {agent.status}{agent.package.version ? ` v${agent.package.version}` : ""}</span></div><h3>{agent.name}</h3><p>{agent.tagline}</p><div className="library-readiness"><strong>{agent.package.readiness.score}</strong><span><b>Readiness</b><small>{agent.package.readiness.evidenceCoverage}% evidence coverage</small></span></div><div className="library-tools">{agent.tools.slice(0, 3).map((tool) => <span key={tool}>{tool}</span>)}</div><div className="library-foot"><span>Edited {new Date(agent.updatedAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span><i>→</i></div></button>)}</div> : <div className="empty-library"><div className="empty-orb">✦</div><h2>Your first agent starts with one sentence.</h2><p>Describe the outcome you want. Milana Architect will configure everything else with you.</p><button onClick={onNew}>Create your first agent</button></div>}</div>;
}
