  ...
  return (
    <main className="studio-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-foot"><div className="health-dot"/><div><strong>Kotiba scope active</strong><span>16 agents indexed</span></div></div>
      </aside>
...
function Overview({ agents, data, onOpen }: { agents: Agent[]; data: StudioData; onOpen: () => void }) {
  ...
  return <div className="view overview-view">
    <section className="hero-panel"><div><span className="live-pill"><i/> Current Kotiba architecture</span><h2>Improve the agents already<br/>serving Milana sales.</h2><p>Work with the existing orchestrator and specialists, including the new five-minute catalog follow-up agent.</p><div className="hero-actions"><button className="primary" onClick={onOpen}>Open current registry <span>→</span></button><button className="secondary">View architecture</button></div></div><div className="orbit" aria-label="Current Kotiba agent orchestration diagram"><div className="orbit-ring ring-one"/><div className="orbit-ring ring-two"/><div className="orbit-core"><span>1 + 15</span><small>current system</small></div>{["Intent","Memory","Media","Reasoning","Follow-up","Audit"].map((x,i)=><span key={x} className={`orbit-node node-${i+1}`}>{x}</span>)}</div></section>
    <section className="metric-grid"><Metric label="Current agents" value={String(agents.length)} detail="1 orchestrator · 15 specialists" tone="violet"/><Metric label="Saved prompt versions" value={String(data.versions.length)} detail="Durable workspace history" tone="blue"/><Metric label="Evaluation runs" value={String(data.runs.length)} detail="Grounding · language · sales" tone="gold"/><Metric label="Follow-up delay" value="5m" detail="Only when customer stays silent" tone="rose"/></section>
    <section className="overview-bottom"><div className="panel squad-panel"><div className="panel-head"><div><p className="eyebrow">CURRENT CAPABILITY MAP</p><h3>{squads.length} groups, one orchestrator</h3></div><button onClick={onOpen}>View all →</button></div><div className="squad-cloud">{squads.map((name, i)=><div key={name} className="squad-chip"><span>{String(i+1).padStart(2,"0")}</span><div><strong>{name}</strong><small>{agents.filter((agent)=>agent.squad===name).length} active components</small></div></div>)}</div></div><div className="panel readiness"><p className="eyebrow">CURRENT SCOPE</p><h3>Existing agents plus follow-up</h3><div className="readiness-score"><span>16</span><small>agents</small></div><div className="progress"><i style={{width:"100%"}}/></div><ul><li className="done">Orchestrator included</li><li className="done">15 specialists mapped</li><li className="done">Five-minute catalog follow-up added</li><li className="done">Cancel conditions defined</li></ul></div></section>
  </div>;
}
...
type FlowTestResult = { runId:string; input:string; output:string; latency:number; attachment:{name:string;format:string;size:string}; followUp:{delay:string;condition:string;output:string;status:string}; path:Array<{label:string;detail:string;status:"passed"|"skipped"}> };
...
const initialFlowEdges: FlowEdge[] = [
  {id:"e1",from:"trigger_1",to:"agent_1"},{id:"e2",from:"agent_1",to:"agent_2"},{id:"e3",from:"agent_1",to:"agent_3"},{id:"e4",from:"agent_2",to:"agent_4"},{id:"e5",from:"agent_3",to:"agent_4"},{id:"e6",from:"agent_4",to:"agent_5"},{id:"e7",from:"agent_4",to:"tool_catalog"},{id:"e8",from:"agent_4",to:"agent_6"},{id:"e9",from:"agent_5",to:"output_1"},{id:"e10",from:"tool_catalog",to:"output_1"},{id:"e11",from:"agent_6",to:"output_1"},{id:"e12",from:"tool_catalog",to:"agent_7"},{id:"e13",from:"agent_7",to:"output_followup"},
];
...
function FlowBuilder({ agents, notify }: { agents:Agent[]; notify:(text:string)=>void }) {
  ...
  const runFlow=()=>{
    ...
    window.setTimeout(()=>{
      ...
      setTestResult({
        ...
        followUp:{delay:"5 minutes after catalog",condition:"Send only if no customer reply, handoff, opt-out, or order activity",output:"Katalogni ko‘rib chiqishga ulgurdingizmi? Sizga yoqqan modelning rasmi yoki artikulini yuborsangiz, narxi va buyurtma shartlarini tekshirib beraman.",status:"Scheduled"},
        latency:1840,
        path,
      });
      setRunning(false);
  ...
  return <div className="view flow-view">
    ...
    <div className="flow-layout">
      <aside className="node-palette"><p>ADD NODE</p>{nodeTypes.map((item)=><button key={item.type} onClick={()=>addNode(item.type)}><i className={`node-icon ${item.type}`}>{item.mark}</i><span><strong>{item.label}</strong><small>{item.type==="agent"?"Choose from 16 current agents":item.type==="knowledge"?"Uploaded files and catalog":item.type==="guardrail"?"Validate before next step":"Workflow building block"}</small></span><b>＋</b></button>)}</aside>
