  ...
  return <div className="view flow-view">
    ...
    {(running||testResult)&&<section id="flow-test-output" className={`flow-test-output ${running?"is-running":""}`} aria-live="polite">
      {running?<div className="run-progress"><span className="run-spinner"/><div><p className="eyebrow">TEST RUN IN PROGRESS</p><h3>Executing the current workflow…</h3><p>Tracing every agent decision. No live Instagram message will be sent.</p></div></div>:testResult&&<>
        <header className="run-result-head"><div><span className="run-status">✓ Passed</span><h3>Simulation output</h3><p>Review each public reply, private reply, delivery receipt, and workflow decision before release.</p></div><div className="run-meta"><span><small>Run</small><strong>{testResult.runId}</strong></span><span><small>Latency</small><strong>{(testResult.latency/1000).toFixed(2)}s</strong></span><span><small>Steps</small><strong>{testResult.path.length}</strong></span><span><small>Live sends</small><strong>0</strong></span></div></header>
